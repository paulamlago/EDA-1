...
unsigned int aux(const Arbin<int>& a, unsigned int k, unsigned int cont) {

	if (a.esVacio()) return 0;

	if (a.hijoDer().esVacio() && a.hijoIz().esVacio()) return cont >= k;

	return aux(a.hijoDer(), k, cont+1) + aux(a.hijoIz(), k, cont+1);
...
/**
  COMPLEJIDAD: Explica aquí justificadamente cuál es la complejidad de 
		...
		- O(n) respecto al número de nodos, ya que tan solo pasamos una vez por acada nodo
