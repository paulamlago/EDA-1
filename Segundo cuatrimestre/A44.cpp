...
List<int> initializeAlumnos(long long int dim);
int selecciona(long long int n, long long int m);
...
int selecciona(long long int n, long long int m) {
	List <int> alumnos = initializeAlumnos(n);
	long long int j, e, i = 1;

	while (i < n && alumnos.size() != 1) { //cuado la lista tiene un solo elemento es cuando queremos salir
		j = 1;
		while (j < m) {
			e = alumnos.front();
			alumnos.pop_front();
