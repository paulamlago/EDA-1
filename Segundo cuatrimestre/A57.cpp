...
void fillArbin(Arbin<int> &i);
bool correcto(Arbin<int> i, int &altura);
...
bool correcto(Arbin<int> i, int &altura) {
	...
	if (!i.esVacio()) {
		if (i.hijoDr().esVacio() && i.hijoIz().esVacio()) { //si es hoja -> okay
			altura = 1;
			return true;
		}

		if (!i.hijoIz().esVacio()){ //comprobamos que la diferencia de edad con el padre para ambos hijos es +18
			if (i.raiz() < i.hijoIz().raiz() + 18) {
				correct = false;
			}
			if (correct && !i.hijoDr().esVacio()) {
				if (i.raiz() < i.hijoDr().raiz() + 18) {
					correct = false;
				}
			}
		}
		
		if (correct) { //comprobamos que si hay dos hijos se lleven más de 2 años
			if (!i.hijoIz().esVacio() && !i.hijoDr().esVacio()) {
				if (i.hijoIz().raiz() < i.hijoDr().raiz() + 2) {
					correct = false;
				}
			}
		}
		...
		if (correct) {
			bool iz = correcto(i.hijoIz(), hi);
			bool dr = correcto(i.hijoDr(), hd);
			if (hi == hd) altura = hi + 1;
			else if (hi < hd) altura = hd + 1;
			else altura = hi + 1;
