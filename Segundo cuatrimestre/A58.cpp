...
void tesoro(Arbin<int> i, tTesoro &t);
...
void tesoro(Arbin<int> i, tTesoro &t) {
	//el arbol nunca va a estar vacío
	//el tesoro está en la raiz

	tTesoro iz = t, dr = t;

	if (!i.hijoIz().esVacio() || !i.hijoDr().esVacio()) {
		if (!i.hijoIz().esVacio()) {
			if (i.raiz() == 1) iz.dragones++;
			tesoro(i.hijoIz(), iz);
		}
		if (!i.hijoDr().esVacio()) {
			if (i.raiz() == 1) dr.dragones++;
			tesoro(i.hijoDr(), dr);
		}

		if (iz.num != -1 && dr.num != -1) {
			if (iz.dragones == dr.dragones) t = iz;
			else {
				if (iz.dragones < dr.dragones) t = iz;
				else t = dr;
			}
		}
		else if (iz.num == -1) t = dr;
		else t = iz;
	}
	else { //es hoja
		t.num =  i.raiz();
