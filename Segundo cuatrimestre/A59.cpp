...
int completo(Arbin<int> i, bool &c);
...
int completo(Arbin<int> i, bool &c) {
	if (!i.esVacio()) {
		...
		if (!i.hijoIz().esVacio() && !i.hijoDr().esVacio()) { //si tiene los dos hijos
			ai = completo(i.hijoIz(), ci);
			ad = completo(i.hijoIz(), cd);
		}
		else { //si solo tiene un hijo o ninguno entonces no es completo
			if (!i.hijoIz().esVacio()) {
				ai = completo(i.hijoIz(), ci);
			}
			else if (!i.hijoDr().esVacio()) {
				ad = completo(i.hijoIz(), cd);
			}
			else { //si es hoja
				c = false;
