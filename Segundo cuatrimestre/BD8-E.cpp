...
int main() {
	...
	while (!cin.eof()) {
		getline(cin, cadena);
			eq = esEquilibrada(cadena);

			if (eq) cout << "EQUILIBRADA";
			else cout << "NO_EQUILIBRADA";
			cout << endl;
	}
...
bool esEquilibrada(const string& cadena) {
	...
	unsigned int i = 0;
	while (i < cadena.length() && eq) {

		if (cadena[i] == '(' || cadena[i] == '[' || cadena[i] == '{' || cadena[i] == ')' || cadena[i] == ']' || cadena[i] == '}') {
			if (cadena[i] == ')' || cadena[i] == ']' || cadena[i] == '}') { //si cierran el top tiene que ser su apertura
				if (!stack.empty()) {
					if (cadena[i] == ')' && stack.top() != '(') eq = false;
					else if (cadena[i] == ')' && stack.top() == '(') stack.pop();
					else if (cadena[i] == ']'&& stack.top() != '[')eq = false;
					else if (cadena[i] == ']'&& stack.top() == '[')stack.pop();
					else if (cadena[i] == '}' && stack.top() != '{') eq = false;
					else if (cadena[i] == '}' && stack.top() == '{') stack.pop();
				}
			...
			else stack.push(cadena[i]); //abren
		}
		i++;
	}

	return eq && stack.empty();
}