	...
	*/
	...
	void eliminarElemento(int n) {
		...
		while (recorrer != NULL) {
			if (recorrer->_elem == n) {
				if (recorrer == _prim) {
					if (_prim == _ult) { //solo hay un elemento
						_prim = NULL;
						_ult = NULL;
						_numElems = 0;
						recorrer = recorrer->_sig;
					}
					else { //borramos ese primer elemento
						recorrer = recorrer->_sig;
						recorrer->_ant = NULL;
						_prim = _prim->_sig;
						_prim->_ant = NULL;
				...
				else if (recorrer == _ult) {
					_ult = _ult->_ant;
					_ult->_sig = NULL;
					recorrer = recorrer->_sig; //NULL;
					_numElems--;
				}
				else { //estamos borrando un elemento del medio
					recorrer->_ant->_sig = recorrer->_sig;
					recorrer->_sig->_ant = recorrer->_ant;
					_numElems--;
					recorrer = recorrer->_sig;
				}
			}
			else recorrer = recorrer->_sig;
		}
	}
