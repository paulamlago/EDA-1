...
void fillTreeMap(Arbin<int> a, List<int> pre, List<int> in, int &posP, int &posI);
...
int main() {
	...
	while (!preOrden.empty() && !inOrden.empty()){
		Arbin<int> a = Arbin<int>(preOrden.at(0));
		int pos = 0; 
		int pos2 = 0;
		fillTreeMap(a, preOrden, inOrden, pos, pos2);
		List<int> sol = a.niveles();
		List<int>::ConstIterator it = sol.cbegin();
		while (it != sol.cend()){
			cout << it.elem();
			it.next();
			if (it != sol.cend()) cout << " ";
...
void fillTreeMap(Arbin<int> a, List<int> pre, List<int> in, int &posP, int &posI){
	...
	if (pre.at(posP) != in.at(posI)){
		a.hijoIz() = Arbin<int>(pre.at(posP));
		pre.pop_front();
		posP++;
		fillTreeMap(a.hijoIz(), pre, in, posP, posI);
	}
	//else has rellenado toda la rama izquierda
	else{
		in.pop_front(); //la raiz de ese arbol ya la hemos introducido
