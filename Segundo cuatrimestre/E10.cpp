...
public:
	...
	*/
	...
	void getLeaves(int *leaves, int &count) {
		Nodo *aux = _ra;

		if (!esVacio()){
			if (aux->_iz == NULL && aux->_dr == NULL) {
				leaves[count] = aux->_elem;
				count++;
			}
			else {
				Nodo *saveDr = _ra->_dr;
...
//void getLeaves(const Arbin<int> &arbol, int *l, int &count);
...
int main() {
	...
	while (n != 0) {
		...
		int *leaves = new int[arbol.numHojas()];
		count = 0;

		//getLeaves(arbol, leaves, count);

		arbol.getLeaves(leaves, count);
...
/**void getLeaves(const Arbin<int> &arbol, int *l, int &count) {
	if (!arbol.esVacio()) {
		if (arbol.hijoIz().esVacio() && arbol.hijoDr().esVacio()) {
			...
			getLeaves(arbol.hijoIz(), l, count);
			getLeaves(arbol.hijoDr(), l, count);
		}
	}
}*/