...
template<typename T>
void fillArbin(Arbin<T> &arbol, const T stopMark);
template<typename T>
T menor(Arbin<T> arbol);
...
int main() {
	...
	while (!cin.eof()) {
		...
		if (tipo == 'N') {
			Arbin<int> arbol;
			fillArbin(arbol, -1);
			if (!arbol.esVacio()) cout << menor(arbol);
			cout << endl;
		}
...
void fillArbin(Arbin<T> &arbol, const T stopMark) {
	...
	if (x != stopMark) {
		fillArbin(hijoIzq, stopMark);
		fillArbin(hijoDrch, stopMark);

		arbol = Arbin<T>(hijoIzq, x, hijoDrch);
	}
}
...
T menor(Arbin<T> arbol) {
		...
		if (arbol.hijoDr().esVacio() && arbol.hijoIz().esVacio()) return arbol.raiz(); //una hoja!
		else {
				T elemIz, elemDr;

				if (!arbol.hijoIz().esVacio())  elemIz = menor<T>(arbol.hijoIz());
				if (!arbol.hijoDr().esVacio())  elemDr = menor<T>(arbol.hijoDr());

				if (!arbol.hijoIz().esVacio() && !arbol.hijoDr().esVacio()) { //si tiene las dos ramas
					if (elemIz == elemDr && elemDr = arbol.raiz()) return elemDr; //si son todos iguales
					else if (elemIz == elemDr && elemDr < arbol.raiz()) return elemDr; //si la rama izq y dch son iguales
					else if (elemIz == arbol.raiz() && elemDr < arbol.raiz()) return elemDr;
					else if (elemDr == arbol.raiz() && elemIz < arbol.raiz()) return elemIz;
					else if (elemIz < elemDr && elemIz < arbol.raiz()) return elemIz;
					else if (elemDr < elemIz && elemDr < arbol.raiz()) return elemDr;

					else return arbol.raiz();
				}
				else if (!arbol.hijoIz().esVacio() && arbol.hijoDr().esVacio()) return (elemIz < arbol.raiz() ? elemIz : arbol.raiz()); //si solo tiene la rama izq
				else return (elemDr < arbol.raiz() ? elemDr : arbol.raiz()); //si solo tiene la rama drch
		}
	}
