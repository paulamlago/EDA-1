...
int navegables(Arbin<int> arbol, int &count);
...
int main() {
	...
	while (n != 0) {
		...
		b = navegables(arbol, count);

		if (b >= 3) { //la súma del último tramo no la contamos
			count--;
		}
...
int navegables(Arbin<int> arbol, int &count) {
	if (!arbol.esVacio()) {
		if (arbol.hijoDr().esVacio() && arbol.hijoIz().esVacio()) { //es hoja
			/**int af = 1 - arbol.raiz();
			if (af > 0) return af; //no puede ser negativo pero sí 0
			else return 0;*/
			return 1;
		}
		else {
			int affIz = navegables(arbol.hijoIz(), count);
			int affDr = navegables(arbol.hijoDr(), count);
			if ((affIz + affDr) - arbol.raiz() >= 3) count++;
			return (affIz + affDr) - arbol.raiz() > 0 ? (affIz + affDr) - arbol.raiz() : 0;
		}
	}
	else return 0;
}