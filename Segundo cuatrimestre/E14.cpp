...
void fillArbin(Arbin<int> &arbol);
int profundidadMultiplo7(Arbin<int> arbol, int &profundidad);
...
int main() {
	...
	while (n != 0) {
		...
		fillArbin(arbol);
		num = profundidadMultiplo7(arbol, p);

		if (num != -1) cout << num << " " << p;
		else if (num == -1) cout << "NO HAY";
...
void fillArbin(Arbin<int> &arbol) {
	...
	if (x != -1) {
		...
		arbol = Arbin<int>(hijoIzq, x, hijoDrch);
	}
}

int profundidadMultiplo7(Arbin<int> arbol, int &profundidad) {

	if (!arbol.esVacio() && !isPrime(arbol.raiz())) {
		if (arbol.raiz() % 7 == 0 && !isPrime(arbol.raiz())) return arbol.raiz();
		
		else {
			profundidad++;
			int izq = -1, dr = -1, pi = profundidad, pd = profundidad;
			
			izq = profundidadMultiplo7(arbol.hijoIz(), pi);
			dr = profundidadMultiplo7(arbol.hijoDr(), pd);
			...
			else { //los dos son != -1 o solo el dr es -1
				profundidad = pi;
				return izq;
			}
		}
	...
	/**if (!arbol.esVacio() && !isPrime(arbol.raiz())) {
		...
		if (arbol.hijoDr().esVacio() && arbol.hijoIz().esVacio()) { //es hoja
			if (arbol.raiz() % 7 == 0 && !isPrime(arbol.raiz())) {
				return arbol.raiz();
			}
			else return -1;
		}
		else {
			int profIz = 1, profDr = 1, solIz, solDr;

			solIz = profundidadMultiplo7(arbol.hijoIz(), profIz);
			solDr = profundidadMultiplo7(arbol.hijoDr(), profDr);
