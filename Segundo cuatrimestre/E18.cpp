...
//ejercicio resuelto con Tree Maps
/**void fillList(List<string> &lista);
void fillArbin(List<List<string>> texto, TreeMap<string, List<int>> &arbol);
void printArbin(TreeMap<string, List<int>>::ConstIterator it, TreeMap<string, List<int>>::ConstIterator itEnd);
...
int main() {
	...
	while (n != 0) {
		...
		fillArbin(texto, arbol);
		TreeMap<string, List<int>>::ConstIterator it = arbol.cbegin();
		TreeMap<string, List<int>>::ConstIterator itEnd = arbol.cend();
...
void fillArbin(List<List<string>> texto, TreeMap<string, List<int>> &arbol) {
	List<List<string>>::Iterator it = texto.begin();
	List<List<string>>::Iterator itEnd = texto.end();
	int counter = 1;

	while (it != itEnd) {
		List<string> linea = it.elem();
		List<string>::Iterator itLinea = linea.begin();
		List<string>::Iterator itLineaEnd = linea.end();

		while (itLinea != itLineaEnd) {
			TreeMap<string, List<int>>::Iterator itsInTheTree = arbol.find(itLinea.elem());
			TreeMap<string, List<int>>::Iterator nullIt = arbol.end(); //arbol.end() es NULL

			if (itsInTheTree != nullIt) {
				itsInTheTree.value().push_back(counter);
			}
			else {
				List<int> lineaEnLaQueSeEncuentra;
				lineaEnLaQueSeEncuentra.push_back(counter);
				arbol.insert(itLinea.elem(), lineaEnLaQueSeEncuentra);
			}
			itLinea.next();
		}
...
void printArbin(TreeMap<string, List<int>>::ConstIterator it, TreeMap<string, List<int>>::ConstIterator itEnd) {
	while (it != itEnd) {
		if (it.key().length() > 2) {
			cout << it.key() << " ";
			List<int>::ConstIterator iteradorDeLaLista = it.value().cbegin();
			List<int>::ConstIterator iteradorDeLaListaEnd = it.value().cend();

			int lineaAnterior = 0;
			while (iteradorDeLaLista != iteradorDeLaListaEnd) {
				if (lineaAnterior < iteradorDeLaLista.elem()) cout << iteradorDeLaLista.elem() << " ";
				lineaAnterior = iteradorDeLaLista.elem();
				iteradorDeLaLista.next();
			}
			cout << endl;
		}
		it.next();
...
//ejercicio resuelto con Hash Maps

void fillList(List<string> &lista);
void fillHashMap(HashMap<string, List<int>> &hash, List<List<string>> texto);
void fillTreeMap(HashMap<string, List<int>> hash, TreeMap<string, List<int>> &tree);
void printTree(TreeMap<string, List<int>>::ConstIterator beg, TreeMap<string, List<int>>::ConstIterator end);
...
int main() {
	...
	while (n != 0) {
		...
		//ahora lo introducimos en un hash map
		HashMap<string, List<int>> hash;
		fillHashMap(hash, texto);
		//ahora lo metemos en un treeMap para que esté ordenado
		TreeMap<string, List<int>> tree;
		fillTreeMap(hash, tree);

		TreeMap<string, List<int>>::ConstIterator beg = tree.cbegin();
		TreeMap<string, List<int>>::ConstIterator end = tree.cend();
...
void fillHashMap(HashMap<string, List<int>> &hash, List<List<string>> texto) {
	List<List<string>>::ConstIterator it = texto.cbegin();
	List<List<string>>::ConstIterator itend = texto.cend();
	int lineCounter = 1;
	...
	while (it != itend) {
		...
		while (itLinea != itLineaend) {
			HashMap<string, List<int>>::Iterator found = hash.find(itLinea.elem());
			HashMap<string, List<int>>::Iterator nullPointer = hash.end();

			if (found != nullPointer) {
				found.value().push_back(lineCounter);
				hash.insert(found.key(), found.value());
			}
			else {
				List<int> lineasEnLasQueSeEncuentra;
				lineasEnLasQueSeEncuentra.push_back(lineCounter);
				hash.insert(itLinea.elem(), lineasEnLasQueSeEncuentra);
			}
			itLinea.next();

			if (itLinea == itLineaend) {
				cout << "ffin";
			}
		}
...
void fillTreeMap(HashMap<string, List<int>> hash, TreeMap<string, List<int>> &tree) { //lo metemos en un tree map para imprimirlo ordenado a continuación
	HashMap<string, List<int>>::ConstIterator it = hash.cbegin();
	HashMap<string, List<int>>::ConstIterator end = hash.cend();
...
void printTree(TreeMap<string, List<int>>::ConstIterator beg, TreeMap<string, List<int>>::ConstIterator end) {
	while (beg != end) {
		List<int>::ConstIterator itLista = beg.value().cbegin();
		List<int>::ConstIterator itNULL = end.value().cbegin();
		cout << beg.key() << " ";
		while (itLista != itNULL) {
			cout << itLista.elem() << " ";
			itLista.next();
		}
		cout << endl;
