...
void filltree(TreeMap<string, List<string>> &deportes, TreeMap<string, string> &nombres, string deporte);
void fillFinalTree(TreeMap<string, List<string>>::ConstIterator it, TreeMap<string, List<string>>::ConstIterator end, TreeMap<int, List<string>> &sol);
void printLists(List<int>::ConstIterator beg, List<int>::ConstIterator end, List<string>::ConstIterator ini);
void fillLists(List<int> &apuntados, List<string> &deportes, TreeMap<int, List<string>>::ConstIterator it, TreeMap<int, List<string>>::ConstIterator end);

int main() {
	string  deporte;
	cin >> deporte;

	while (!cin.eof() && deporte != "_FIN_") {
		TreeMap<string, List<string>> deportes; //arbol en el que tendremos cada deporte con los nombres de los alumnos apuntados
		TreeMap<string, string> nombres; //arbol en el que tendremos los nombres de los alumnos junto con el nombre de los deportes

		filltree(deportes, nombres, deporte);

		if (!deportes.empty()) {
			TreeMap<string, List<string>>::ConstIterator it = deportes.cbegin();
			TreeMap<string, List<string>>::ConstIterator itEnd = deportes.cend();
			TreeMap<int, List<string>> result; //en este arbol tendremos el numero de apuntados como clave, y como valor los nombres de los deportes que tienen esa cantidad de alumnos apuntados

			fillFinalTree(it, itEnd, result); //entonces se rellenará el arbol de menor a mayor, en función del número de alumnos apuntados

			//como necesitamos que estén de mayor a menor metemos la información en listas, al revés, invirtiendo totalmente el orden, y luego las imprimimos
			List<int> apuntados;
			List<string> deportesLista;

			TreeMap<int, List<string>>::ConstIterator fit = result.cbegin();
			TreeMap<int, List<string>>::ConstIterator fitEnd = result.cend();

			fillLists(apuntados, deportesLista, fit, fitEnd);

			//imprimimos las listas, que tienen el mismo numero de elementos
			List<int>::ConstIterator beg = apuntados.cbegin();
			List<int>::ConstIterator end = apuntados.cend();
			List<string>::ConstIterator ini = deportesLista.cbegin();

			printLists(beg, end, ini);
...
void filltree(TreeMap<string, List<string>> &deportes, TreeMap<string, string> &nombres, string deporte) {
	string nombre, linea;
		...
		List<string> alumnos;
		deportes.insert(deporte, alumnos);

		while (deporte != "_FIN_"){
			cin >> nombre;
			if (nombre[0] != '_' && islower(nombre[0])) { //es un nombre
				//buscamos ese nombres en el arbol de los nombres, si ya está previamente guardado
				//nos vamos al arbol de deportes y lo borramos de sus listas
				string deporteEnElQueEsta = nombres[nombre];

				if (deporteEnElQueEsta == ""){ //no se encontraba en el arbol
					//metemos este nuevo nombre
					nombres.insert(nombre, deporte);
					List<string> al = deportes[deporte]; //metemos el nombre en la lista de apuntados del deporte
					al.push_back(nombre);
					deportes[deporte] = al;
				}
				else{ //ya estaba guardado en el deporte deporteEnElQueEsta
					//así que nos vamos al arbol y borramos el nombre del chico en la lista de ese deporte
					if (deporteEnElQueEsta != deporte){ //si estaba apuntado en un deporte distinto
						List<string> listaAlumnos = deportes[deporteEnElQueEsta];
						List<string>::Iterator it = listaAlumnos.begin();
						List<string>::Iterator itNULL = listaAlumnos.end();

						bool found = false; //el nombre solo va a aparecer una vez
						while (it != itNULL && found == false){ //recorremos la lista de alumnos
							if (it.elem() == nombre){ //cuando encontramos el nombre lo borramos
								listaAlumnos.erase(it);
								found = true;
							}
							it.next();
						}
						deportes[deporteEnElQueEsta] = listaAlumnos;
					}
				}
			}
			else { //es un deporte nuevo
				deporte = nombre;
				if (deporte != "_FIN_"){
					List<string> alumnos;
					deportes.insert(deporte, alumnos);
...
void fillFinalTree(TreeMap<string, List<string>>::ConstIterator it, TreeMap<string, List<string>>::ConstIterator end, TreeMap<int, List<string>> &sol){
	...
	while (it != end) {
		List<string> apuntados = sol[it.value().size()];

		if (!apuntados.empty()) { //el número ya se encuentra en el arbol
			apuntados.push_back(it.key());
			sol.insert(it.value().size(), apuntados);
		}
		else { //apuntados es una lista vacía
			apuntados.push_back(it.key());
			sol.insert(it.value().size(), apuntados);
		}
		it.next();
...
void fillLists(List<int> &apuntados, List<string> &deportes, TreeMap<int, List<string>>:: ConstIterator it, TreeMap<int, List<string>>::ConstIterator end) {
	while (it != end) {
		unsigned int size = it.value().size();

		while (size > 0) {
			apuntados.push_front(it.key());
			deportes.push_front(it.value().at(size - 1));
			size--;
		}
		it.next();
	}
