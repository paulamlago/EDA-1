...
int AlturaSubArbol(Arbin<int> &i, int &alturaMax);
bool hijos(Arbin<int> i, Arbin<int> d);
...
int main() {
	...
	while (n != 0) {
		...
		fillArbin(i);
		int altura = 0;
		int basura = AlturaSubArbol(i, altura);
		cout << altura << endl;
		n--;
	}
...
int AlturaSubArbol(Arbin<int> &i, int &alturaMax) {
	...
	if (!i.esVacio()) {
		...
		if (!i.hijoIz().esVacio() && !i.hijoDr().esVacio()) {
			hiz = AlturaSubArbol(i.hijoIz(), alturaMax);
			hid = AlturaSubArbol(i.hijoDr(), alturaMax);
		}
		if (hiz == 0 && hid == 0) altura = 1;
		if (hiz == hid) altura = hiz + 1;
		if (hijos(i.hijoIz(), i.hijoDr())) {
			if (hiz < hid) altura = hid;
			else if (hiz > hid) altura = hid;
		}
		else altura = 2;
...
bool hijos(Arbin<int> i, Arbin<int> d) {
	if (!i.esVacio() && !d.esVacio()) {
		bool iz = true, dr = true;

		iz = hijos(i.hijoIz(), d.hijoIz());
		if (iz) dr = hijos(i.hijoDr(), d.hijoDr());
		else return iz;
		if (!dr) return dr;
		return true;
	}
