...
bool zurdo(Arbin<int> i, int &hijos);
...
bool zurdo(Arbin<int> i, int &hijos) {
	if (!i.esVacio()) {
		...
		if (!i.hijoIz().esVacio()) {
			iz = zurdo(i.hijoIz(), hiz);
			if (iz) dr = zurdo(i.hijoDr(), hd);
		}
		else if (i.hijoIz().esVacio() && !i.hijoDr().esVacio()) return false;
		else { //los dos hijos son vacíos
			hijos = 1;
			return true;
		}
		if (hiz > (hiz + hd) / 2 && iz && dr) {
			hijos = hiz + hd + 1;
			return true;
		}
		else return false;
