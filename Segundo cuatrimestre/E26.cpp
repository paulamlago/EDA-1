...
private:
	...
	public:
		Nodo() : _sig(NULL), _ant(NULL) {}
		Nodo(const string &elem) : _elem(elem), _sig(NULL), _ant(NULL) {}
		Nodo(Nodo *ant, const string &elem, Nodo *sig) :
			_elem(elem), _sig(sig), _ant(ant) {}
	...
	HashMap<string, info> canciones;

	List<string> reproduccionesRecientes;
	HashMap<string, Nada> reproducionesRecientesSinOrden; //para comprobar en tiempo constante que no introducimos dos canciones con el mismo nombre a la lista de reproduciones recentes

	List<string> listaReproduccion;
	HashMap<string, Nada> cancionesEnLaListaDeReproduccion; //para poder comprobar si tenemos una cierta cancion en tiempo constante
...
int main() {
	string entrada, orden, cancion, artista;
	int duracion;
	...
	while (!cin.eof()) {
		...
		while (orden != "FIN") {
			if (orden == "addSong") {
				...
				try {
					aparato.addSong(cancion, artista, duracion);
				}
				...
				if (aparato.totalTime() != 0) {
					string c = aparato.current();
					aparato.play();
					cout << "Sonando " << c << endl;
				}
				...
				cin >> cancion;
				aparato.deleteSong(cancion);
			}
			else if (orden == "recent") {
				int N;
				cin >> N;
				List<string> recientes = aparato.recent(N);
				if (!recientes.empty()) {
					cout << "Las " << N << " mas recientes" << endl;
					List<string>::ConstIterator it = recientes.cbegin();
					while (it != recientes.cend()) {
						cout << "    " << it.elem() << endl;
						it.next();
					}
				}
				...
				cout << "Tiempo total " << aparato.totalTime() << endl;
			}
			else if (orden == "current") {
				try {
					string song = aparato.current();
				}
				catch (invalid_argument e) {
					cout << e.what() << endl;
				}
...
void ipud::addSong(const string &s, const string &a, const int &d) {
	if (!canciones.contains(s)) { //constant (canciones is a hash map)
		info i;
		i.artist = a;
		i.duration = d;
		canciones.insert(s, i); //constant time
	}
	else throw invalid_argument("ERROR addSong");
}

void ipud::addToPlaylist(const string &s) {
	if (canciones.contains(s)) {
		if (!cancionesEnLaListaDeReproduccion.contains(s)) { //Es unhash map para que aquí podamos comprobarlo en tiempo constante
			listaReproduccion.push_back(s);
			cancionesEnLaListaDeReproduccion.insert(s, Nada());
			listDuration += canciones[s].duration; //actualizo la duración de mi lista de reproducción
		}
	}
	else throw invalid_argument("ERROR addToPlaylist");
}

string ipud::current() const {
	if (listaReproduccion.size() > 0) { //mi lista no está vacía
		return listaReproduccion.front();
	}
	else throw invalid_argument("ERROR current");
}

void ipud::play() {
	if (listaReproduccion.size() >= 0){
		while (!cancionesEnLaListaDeReproduccion.contains(listaReproduccion.front())) {
			listaReproduccion.pop_front(); //since our list is not updated when a song is deleted but the HashMap, I need to check that the song exists
		}
		if (listaReproduccion.size() >= 0) {
			if (!reproducionesRecientesSinOrden.contains(listaReproduccion.front())) { //si la canción no ha sido reproducida anteriormente entonces la introducimos en reproducciones recientes
				reproducionesRecientesSinOrden.insert(listaReproduccion.front(), Nada());
				reproduccionesRecientes.push_back(listaReproduccion.front()); //esta cancion la introducimos en la lista
			//que está ordenada por orden de reproducción de la más antigua a la última reproducida
			}
			cancionesEnLaListaDeReproduccion.erase(listaReproduccion.front());
			listDuration -= canciones[listaReproduccion.front()].duration;
...
List<string> ipud::recent(const int &N) {
	List<string> reproducidas;
	int counter = 0;

	List<string>::ConstIterator it = reproduccionesRecientes.cbegin();
	while (it != reproduccionesRecientes.cend() && counter < N) {
		if (reproducionesRecientesSinOrden.contains(it.elem())) { //es en el hash map donde tenemos la información correcta de las últimas reproducciones
																// así que si una canción no se encuentra en el hash map es porque ha sido eliminada y no la tenemos en cuenta
			reproducidas.push_front(it.elem());
			counter++;
		}
...
void ipud::deleteSong(const string &s) {
	if (canciones.contains(s)) {
		canciones.erase(s);

		if (cancionesEnLaListaDeReproduccion.contains(s)) { //tendremos que recorrer la lista de punteros para eliminarla
			/**Nodo *aux = listaReproduccion;
			while (aux != ultimaCancionEnLaLista) {
				if (aux->_elem == s) {
			...
			cancionesEnLaListaDeReproduccion.erase(s);
		}
		if (reproducionesRecientesSinOrden.contains(s)) {
			reproducionesRecientesSinOrden.erase(s);
		}
	}
}