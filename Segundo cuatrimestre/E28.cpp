...
#ifndef ventaDeLibros
#define ventaDeLibros

class libro {
private:
	HashMap<string, int> libros;
			//nombre del libo - unidades disponibles
	HashMap<string, int> vecesQueSeHaVendidoCadaLibro;

public:

	void nuevoLibro(const int &n, const string &x);
	void comprar(const string &x);
	bool estaLibro(const string &x) const;
	...
	while (!cin.eof()){
		...
		while (n != 0){
			if (op == "nuevoLibro"){
				...
				List<string> libros = l.top10();
				List<string>::ConstIterator it = libros.cbegin();
				...
				try{
					int num = l.numEjemplares(nombre);
					cout << "Existen " << num << " ejemplares del libro " << nombre << endl;
					cout << "---" << endl;
				}
				catch (invalid_argument e){
					cout << "No existe el libro " << nombre << " en el sistema" << endl << "---" << endl;
				}
			}
			else if (op == "estaLibro"){
				cin.get();
				getline(cin, nombre);
				bool esta = l.estaLibro(nombre);
				if (esta) cout << "El libro " << nombre << " esta en el sistema" << endl;
				else cout << "No existe el libro " << nombre << " en el sistema" << endl;
				cout << "---" << endl;
			}
			else if (op == "eliminLibro"){
				cin.get();
				getline(cin, nombre);
				l.elimLibro(nombre);
			}
...
void libro::nuevoLibro(const int &n, const string &x) { //constante
	if (libros.contains(x)) {
		libros[x] += n;
	}
	else libros.insert(x, n);
}

void libro::comprar(const string &x) {
	if (libros.contains(x)) {
		if (libros[x] > 0) {
			libros[x] = libros[x] - 1;

			if (vecesQueSeHaVendidoCadaLibro.contains(x)) {
				vecesQueSeHaVendidoCadaLibro[x] = vecesQueSeHaVendidoCadaLibro[x] + 1;
			}
			else { 
				vecesQueSeHaVendidoCadaLibro.insert(x, 1);
			}
		}
		else throw std::out_of_range("No hay ejemplares");
	}
	else throw std::invalid_argument("Libro no existente"); //No existe el libro en el sistema
}

bool libro::estaLibro(const string &x) const {
	return libros.contains(x);
}

void libro::elimLibro(const string &x) {
	if (libros.contains(x)) {
		libros.erase(x);
		if (vecesQueSeHaVendidoCadaLibro.contains(x)) vecesQueSeHaVendidoCadaLibro.erase(x);
	}
}

int libro::numEjemplares(const string &x) const {
	if (libros.contains(x)) {
		return libros.at(x); //constant access -> libros[x]
	}
	else throw std::invalid_argument("Libro no existente");
}

List<string> libro::top10() {
	TreeMap<int, List<string>> ventas; //ventas - libros que han tenido ese numero de ventas

	//recorro el hash map de veces que se ha vendido cada libro y lo meto en el tree Map para que se ordene
	HashMap<string, int>::ConstIterator it = vecesQueSeHaVendidoCadaLibro.cbegin();

	while (it != vecesQueSeHaVendidoCadaLibro.cend()) {
	
		TreeMap<int, List<string>>::Iterator itList = ventas.find(it.value());

		if (itList != ventas.end()) {
			itList.value().push_back(it.key());
		}
		else {
			List<string> libro;
			libro.push_back(it.key());
			ventas.insert(it.value(), libro);
		}
		it.next();
	}
	...
	TreeMap<int, List<string>>::ConstIterator itArbol = ventas.cbegin();
	while (itArbol != ventas.cend()) {
		List<string>::ConstIterator books = itArbol.value().cbegin();
		while (books != itArbol.value().cend()) {
			top.push_front(books.elem());
			books.next();
		}
