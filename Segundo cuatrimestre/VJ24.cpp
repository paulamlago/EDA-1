...
struct solucion { bool ok; int sumaNodos; int numNodos; };
void fillArbin(Arbin<int> &arbol);
solucion sumMayorqueNodos(Arbin<int> arbol);
...
solucion sumMayorqueNodos(Arbin<int> arbol) {
	...
	if (!arbol.esVacio()) {
		if (arbol.hijoDr().esVacio() && arbol.hijoIz().esVacio()) { //si es hoja
			sol.numNodos = 1;
			sol.sumaNodos = arbol.raiz();
			sol.ok = (sol.numNodos <= sol.sumaNodos) ? true : false;
		}
		else {
			solucion solIz, solDr;
			solIz = sumMayorqueNodos(arbol.hijoIz());
			solDr = sumMayorqueNodos(arbol.hijoDr());

			sol.sumaNodos = solIz.sumaNodos + solDr.sumaNodos + arbol.raiz();
			sol.numNodos = solIz.numNodos + solDr.numNodos + 1; // + la raiz!!
			sol.ok = solIz.ok && solDr.ok;
			if (sol.ok == true) {
				sol.ok = (sol.numNodos <= sol.sumaNodos) ? true : false;
			}
		}
