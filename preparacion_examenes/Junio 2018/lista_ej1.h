...
public:
	...
	*/
	void repartir() {
	Nodo *_pivote = _prim; // localizamos pivote
	Nodo *_iterad; //iterador
	Nodo *_nextElem = _prim;
	bool primMenor = true;

	// Movemos nuestro primer elemento (pivote) al final de la lista por question de implementacion
		// 1) Hacemos las asignaciones necesarias (movemos las flechitas)
		// 2) Eliminamos los punteros sobrantes

		// Lo hacemos en este order con tal de no perder en ningun momento nuestros nodos

	_prim = _pivote->_sig; 		//a) Hacemos que _prim apunte al segundo elemento de la lista (_pivote->_sig)
	_ult->_sig = _pivote; 		//b) Hacemos que el _sig del ultimo elemento apunte a nuestro (actualmente) primer elemento (_pivote)
	_pivote->_ant = _ult; 		//c) Hacemos que el _ant de nuestro pivote apunte a ultimo
	_ult = _pivote;				//d) Hacemos que _ult apunte al pivote
	_pivote->_sig = nullptr;	//e) Ponemos a nullptr el pintero_sig del pivote (dado que el ultimo elemento no debe tener ninguno detras)
	_prim->_ant = nullptr;		//f) Ponemos el _ant de _prim a nullptr (dado que el primer elemento de la lista no debe tenenr predecesor)

	_iterad = _prim;

	while (_iterad != _pivote) {		// Debe ser _pivote, dado que hemos colocado el pivote al final y modificamos _ult en cada iteraccion donde el elemento iteradoe s mayor a _pivote->_elem

		_nextElem = _iterad->_sig;		// Salvamos el siguiente elemento a iterar para poder modificar _iterad->_sig con libertad

		if (_iterad->_elem >= _pivote->_elem) {	// si el numero iterado es mayor a nuestro pivote, lo ponemos al pinal de la lista

			_ult->_sig = _iterad;
			_iterad->_ant->_sig = _iterad->_sig;
			_iterad->_sig->_ant = _iterad->_ant;
			_iterad->_ant = _ult;
			_ult = _iterad;
			_iterad->_sig = nullptr;

		}
		else {	// si el numero es menor, no hacemos nada (en caso de ser el primer menor, hacemos que _prim apunte a el)
